from flask import Flask, flash , redirect, render_template , request, session, abort , Markup, url_for
import os
import pandas as pd
import numpy as np
...
def preprocess_data_default():
    dataset = pd.read_csv('Churn_Modelling.csv')
    fpath = os.path.join("default", "uploadtest.csv")
...
def predict_default(z):
     K.clear_session()
     test = preprocess_data_default()
     model = load_model('mlogouty_model.h5')
     model._make_predict_function()
     y_pred = model.predict(test)
     fpath = os.path.join("default", "testtest1.csv")
     dff = pd.read_csv(fpath)
     dff['Exited'] = y_pred
     dff.set_index('RowNumber', inplace=True)
     dff.sort_values('Exited', ascending=False, inplace=True)
